	...
	\begin{block}{How to assess the performance of the model?}
		...
		\begin{center}
			\begin{tikzpicture}
				...
				\node at (-1.5,-0.325) {$\Delta^k$ too large};
...
\begin{frame}{Unconstrained trust-region framework}
	\begin{algorithm}[H]
        ...
        Define~$m^k$ and calculate~$d^k$ by solving~$ \min_{\norm{d} \le \Delta^k} \obj(x^k + d)$\;
        ...
        \begin{empheq}[left={\Delta^{k + 1} \gets \empheqlbrace}]{alignat*=2}
